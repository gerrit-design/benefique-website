...
function Blog() {
  ...
  const posts = [
    { title: 'The Magnet-Hour: What an Imaging Slot Is Actually Worth', excerpt: 'We built one imaging network a payer ranking and told them to protect their best-paying slots. Then the protected slots lost more bookings than the ones we had deprioritized — 15% against 10%. Not because the ranking was wrong. Because we had ranked on price, and a slot is only worth what shows up.', date: '2026-08-29', slug: 'revenue-per-available-magnet-hour', published: true, category: 'Healthcare Finance', readTime: '12 min read' },
    ...
    { title: '$3.64M or $2.45M? The Same Business, Two Profit Numbers, Both Correct', excerpt: 'A 3-partner Florida law firm we work with closed April 2026 with $3.64M of profit and $2.45M of profit. Both numbers are correct. The $1.19M gap between them has three names — and confusing them is how owners over-distribute, under-fund tax, and get blindsided in March.', date: '2026-05-10', slug: 'cash-vs-accrual-small-business', published: true, category: 'Cash Flow', readTime: '9 min read' },
    { title: 'The Cash Conversion Cycle Lie: Why a Blended CCC Hides Everything', excerpt: 'A profitable veterinary practice in Atlanta tracks a 27-day Cash Conversion Cycle and feels good about it. The problem is that 27 days is an average — and the streams underneath it move in opposite directions. One stream funds the business. The other one bleeds it. The blended number tells you neither.', date: '2026-05-09', slug: 'cash-conversion-cycle-lie', published: true, category: 'Cash Flow', readTime: '7 min read' },
    { title: 'Why Your Record Month Drained Your Bank Account', excerpt: 'A Charlotte HVAC contractor closed the strongest month in the firm\'s history — $293K of revenue and $116K of net income. The bank balance went down. The owners stared at the statement and asked the most common question in small business: where did the money go? The math is brutal, and the fix is a single policy.', date: '2026-05-09', slug: 'record-month-drained-bank-account', published: true, category: 'Cash Flow', readTime: '6 min read' },
    { title: 'Cash Machine or Exit Machine: The Owner\'s Decision', excerpt: 'Every owner-operator eventually faces the same fork: optimize the business to pay me forever, or position it to sell. Most never make the decision explicitly. They drift. The drift costs them millions — and the framework for deciding is simpler than the M&A industry wants you to believe.', date: '2026-05-06', slug: 'cash-machine-vs-exit-machine', published: true, category: 'Business Strategy', readTime: '9 min read' },
    ...
    { title: 'Multi-Entity P&L: Why Your Blended Margin Is Lying (15% vs 50%)', excerpt: 'A $3M distribution business showed a 15% gross margin and looked distressed. We split it into internal and external divisions and found a healthy 50%+ external business sitting next to a subsidized 9% internal transfer operation. The owner had been trying to fix the wrong problem for two quarters.', date: '2026-04-14', slug: 'blended-pl-lying-multi-entity-business', published: true, category: 'Cash Flow', readTime: '10 min read' },
    ...
    { title: 'Concierge Doctor Income: 300 Patients = $430K Take-Home (2026)', excerpt: '300 patients × $350/mo = $1.26M gross. After $356K overhead, W-2 salary, payroll tax, Cash Balance Plan, and federal tax: $430K take-home in 2026. Every dollar of the math, step by step.', date: '2026-04-08', slug: 'concierge-doctor-300-patient-panel-income', published: true, category: 'Concierge Medicine', readTime: '14 min' },
    ...
    { title: 'EBITDA Positive but Cash Flow Negative? The Debt-Service Math', excerpt: 'Five locations. $16.7M in revenue. $332K in EBITDA. Looks like a functional business \u2014 until you subtract the $365K in annual interest. The group literally cannot pay its lenders from operations.', date: '2026-04-03', slug: 'ebitda-positive-cash-flow-negative-debt-service', published: true, category: 'Cash Flow', readTime: '16 min' },
    ...
    { title: 'Cash Flow Waterfall Explained: From $1.07M EBITDA to \u2013$41K Cash', excerpt: '$1.07M EBITDA, \u2013$41K cash. The waterfall shows exactly where every dollar went \u2014 operating cash, debt service, distributions, working capital. Real $5M imaging-center walkthrough, line by line.', date: '2026-03-18', slug: 'ai-cash-flow-waterfall-explained', published: true, category: 'Cash Flow', readTime: '12 min' },
    { title: 'Are Concierge Medical Fees Tax Deductible? The 2026 Answer (HSA, FSA, Schedule A)', excerpt: 'Direct answer for patients and CPAs: when MDVIP and concierge fees ARE deductible, when they are not, and how HSA, FSA, and Schedule A rules apply for 2026.', date: '2026-03-17', slug: 'concierge-medical-fees-tax-deductible', published: true, category: 'Tax Strategy', readTime: '18 min' },
    ...
    { title: 'How AI Found $353K of Trapped Cash Inside a Healthcare Practice Balance Sheet', excerpt: 'A real working capital diagnosis: 4 hidden cash leaks, a 6-week recovery plan, and the AI workflow that surfaced $353K already sitting in the books.', date: '2026-03-12', slug: 'ai-found-353k-trapped-cash', published: true, category: 'Cash Flow', readTime: '11 min' },
    ...
    { title: 'How Much Does It Cost to Start a Concierge Practice? $75K–$200K (2026)', excerpt: '$75K–$200K to launch a concierge medical practice in 2026. Most Broward County physicians land at $100K–$175K. Full line-item breakdown: entity setup, buildout, EHR, marketing, $19,750/mo fixed costs, and the S-Corp election deadline most consultants forget.', date: '2026-03-08', slug: 'cost-starting-concierge-medical-practice', published: true, category: 'Concierge Medicine', readTime: '22 min' },
    { title: 'Concierge Medicine Income in South Florida: 2026 Salary Benchmarks', excerpt: 'What South Florida concierge doctors actually earn in 2026, broken down by panel size, fee structure, and Davie/Broward market data. Net $500K to $1M+ with 200 to 500 patients.', date: '2026-03-07', slug: 'concierge-medicine-income-south-florida', published: true, category: 'Concierge Medicine', readTime: '16 min' },
...
// Slots come from src/booking-slots.json, regenerated weekly by
// scripts/update-booking-slots.py from Gerrit's Google Calendar.
// ============================================================
const BOOKING_SLOTS = BOOKING_DATA.slots || [];
...
function groupSlotsByDate(slots) {
  ...
  for (const s of slots) {
    if (!seen.has(s.date)) {
      seen.set(s.date, groups.length);
      groups.push({ date: s.date, slots: [] });
    }
    groups[seen.get(s.date)].slots.push(s);
  }
  return groups;
}
...
function ThankYou() {
  ...
  const slotGroups = groupSlotsByDate(BOOKING_SLOTS);
  const hasSlots = slotGroups.length > 0;
  const gridCols = slotGroups.length >= 2 ? 'md:grid-cols-2' : 'md:grid-cols-1';
                ...
                <div>
                  ...
                  <div className={`grid ${gridCols} gap-6`}>
                    {slotGroups.map((group) => (
                      <fieldset key={group.date}>
                        <legend className="font-semibold text-benefique-navy mb-3">{group.date}</legend>
          ...
          <div className="max-w-2xl mx-auto px-4 text-center">
            <div className="bg-white rounded-2xl p-8 border border-gray-100">
              <p className="text-gray-700">
                No open slots this week — Gerrit will be in touch within 24 hours to find a time.
              </p>
            </div>
          </div>
        </section>
